...
uint8_t RunDemoApplicationPingPong( void )
{
...
#ifdef PRINT_DEBUG
        printf( "Start RunDemoApplicationHtsens.\n\r" );
#endif
    ...
    if( Eeprom.EepromData.DemoSettings.Entity == MASTER )
    {
        switch( DemoInternalState )
        {
            case SEND_PING_MSG:
                ...
                else
                {
                    if( SendNext == true )
                    {
                        ...
                        Buffer[6] = PingMsg[2];
                        Buffer[7] = PingMsg[3];
                        for( i = 8; i < Eeprom.EepromData.DemoSettings.PayloadLength; i++ ) /* @@@ Padding */
                        {
                            Buffer[i] = i;
                        }
                        TX_LED = !TX_LED;
            ...
            case APP_RX:
                //DemoSensors->Sensor[0].data_a2 += 1;
    ...
    else // SLAVE
    {
        switch( DemoInternalState )
        {
            ...
            case APP_IDLE: // do nothing
                ...
                /* @@@ Modif de test. Force le refresh des capteurs à afficher en local */
                refreshDisplay = 1;
...
void ComputeHtsensPayload( uint8_t *buffer, uint8_t bufferSize )
{
    uint32_t i = 0;
...
/*!
 * @@@ Desc TODO Sensor App
 */

DemoSensors_t *DemoSensors;
PinName SensorNssPin[] = { D0, D1, D14, D15 };

uint16_t DemoEnabledSensorMask = 0x0F /* 0000000000001111 : Enabled sensor that can be displayed by user */;          
uint16_t DemoSelectedSensorsMask = 0x03;

uint8_t InitSensors()
{
    uint8_t SensorCount = /*CountSetBits( DemoEnabledSensorMask )*/ 4;
    DemoSensors = ( DemoSensors_t * )calloc( 1, sizeof( DemoSensors_t ) );
    DemoSensors->SensorCount = SensorCount;
    DemoSensors->Sensor = ( DemoSensorData_t * )calloc( SensorCount, sizeof( DemoSensorData_t ) );
    
    return SensorCount;
}

void RefreshSelectedSensors()
{
    Sensor *sensor_spi;
    for ( int i = 0; i < 16; i++ )
    {
        if( ( ( DemoSelectedSensorsMask & DemoEnabledSensorMask ) >> i ) & 0x01 )
        {
            /* Sensor sensor_spi(D11, D12, D13, SensorNssPin[i]); */
            sensor_spi = new Sensor(D11, D12, D13, SensorNssPin[i]);
            /* DemoSensors->Sensor[i].data_a1 = sensor_spi.Read_Register(0xd74); */
            DemoSensors->Sensor[i].data_a2 = sensor_spi->Read_Register(0xd75);
            DemoSensors->Sensor[i].data_temp = sensor_spi->Read_Register(0xd76);
            delete sensor_spi;
        }
    }
    return;
...
void RefreshEnabledSensors()
{
    Sensor *sensor_spi;
    for ( int i = 0; i < 16; i++ )
    {
        if( ( DemoEnabledSensorMask >> i ) & 0x01 )
        {
            sensor_spi = new Sensor(D11, D12, D13, SensorNssPin[i]);
            /* DemoSensors->Sensor[i].data_a1 = sensor_spi.Read_Register(0xd74); */
            DemoSensors->Sensor[i].data_a2 = sensor_spi->Read_Register(0xd75);
...
void RotateLeftSelectedSensors()
{
    uint16_t TempDemoSelectedSensorMask = 0x00;
    int jRelative;
    for ( int i = 0; i < 16; i++ ) /* @@@ Point every bits of the sensor masks */
    {
        if( ( ( DemoSelectedSensorsMask & DemoEnabledSensorMask ) >> i ) & 0x01 ) /* @@@ Check if sensor ID is enabled and currently selected for diplay */
        {
            for ( int j = 1; j < 16; j++ )                                  /* @@@ Let's find the next enabled sensor ID to display (left rotation) */
            {
                jRelative = (i+j)%16;                                       /* @@@ Point every bits starting at position i of the sensor masks */
                
                if( ( ( DemoEnabledSensorMask >> jRelative ) & 0x01 ) )     /* @@@ Check if next sensor is enabled for display */
                {
                    TempDemoSelectedSensorMask |= ( 1 << jRelative );      /* @@@ Select next sensor for display */
                    break;                                                 /* @@@ End scanning */
                }
            }
        }
        else /* @@@ ID not enabled for display, do not change the selection mask */
        {
            if ( ( DemoSelectedSensorsMask >> i ) & 0x01 ) /* @@@ TODO : Improve to full bitwise operations. No 'if' statement pls... */
            {
                TempDemoSelectedSensorMask |= 4U << i;
            }
        }
    }
    DemoSelectedSensorsMask = TempDemoSelectedSensorMask;
    return;
}

void RotateRightSelectedSensors()
{
    uint16_t TempDemoSelectedSensorMask = 0x00;
    int jRelative;
    for ( int i = 0; i < 16; i++ ) /* @@@ Point every bits of the sensor masks */
    {
        if( ( ( DemoSelectedSensorsMask & DemoEnabledSensorMask ) >> i ) & 0x01 ) /* @@@ Check if sensor ID is enabled and currently selected for diplay */
        {
            for ( int j = 15; j > 0; j++ )                                  /* @@@ Let's find the next enabled sensor ID to display (left rotation) */
            {
                jRelative = (i+j)%16;                                         /* @@@ Point every bits starting at position i of the sensor masks */
...
/* Return the selected sensor Id */
uint8_t GetSelectedSensorId(uint8_t offset)
{
    for ( int i = 0; i < 8; i++ ) /* Scan all bits of the mask */
    {
        if( ( ( DemoSelectedSensorsMask & DemoEnabledSensorMask ) >> i ) & 0x01 )
        {
            if( !offset-- )
...
uint8_t GetLastEnabledSensorId()
{
    uint8_t last;
    for ( int i = 0; i < 8; i++ )
    {
        if ( ( DemoSelectedSensorsMask >> i ) & 0x01 )
        {
            last = i;
        }
    }
...
/* @@@ TODO */
uint8_t GetLastSelectedSensorId()
{
    uint8_t last;
    for ( int i = 0; i < DemoSensors->SensorCount; i++ )
    {
        GetSelectedSensorId(i);
    }
    return last;
}
...
// *                                                                           *
// *****************************************************************************
uint8_t RunDemoApplicationHtsens( void )
{
    uint8_t i = 0;
    uint8_t refreshDisplay = 0;
    ...
    if( Eeprom.EepromData.DemoSettings.Entity == MASTER )
    {
        switch( DemoInternalState )
        {
            case SEND_PING_MSG:
                ...
                else
                {
                    if( SendNext == true )
                    {
                        ...
                        Buffer[7] = PingMsg[3];
                        */
                        /*for( i = 0; i < DemoSensors->SensorCount; i++ )
                        {
                            Buffer[4+i] = 0;
                        }*/
                        ...
                        IrqMask = IRQ_TX_DONE | IRQ_RX_TX_TIMEOUT;
                        Radio.SetDioIrqParams( IrqMask, IrqMask, IRQ_RADIO_NONE, IRQ_RADIO_NONE );
                        Radio.SendPayload( Buffer, /*Eeprom.EepromData.DemoSettings.PayloadLength*/ 4, \
                                                   ( TickTime_t ){ RX_TIMEOUT_TICK_SIZE, \
                                                   Eeprom.EepromData.DemoSettings.TimeOnAir + RX_TX_TRANSITION_WAIT } );
                    }
                }
            ...
            case APP_RX:
                ...
                /* @@@ Copy Sensor data from payload to local */
                for( i = 0; i < ( DemoSensors->SensorCount * 4 ); i++ ) /* x4 due to two bytes of A2 data and two bytes of temp data */
                {
                    ((uint8_t*)DemoSensors->Sensor)[i] = Buffer[i+8];
                }
    ...
    else // SLAVE
    {
        switch( DemoInternalState )
        {
            case SEND_PONG_MSG:
                ...
                Buffer[11] = PongMsg[3];
                */
                for( i = 0; i < ( DemoSensors->SensorCount * 4 ); i++ )
                {
                    Buffer[8+i] = ((uint8_t*)DemoSensors->Sensor)[i];
                }
            ...
            case APP_RX:
                ...
                // RefreshSelectedSensors(); /* refresh on Master request */
                RefreshEnabledSensors();
                break;
