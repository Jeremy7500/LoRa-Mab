 ...
 */
#define FIRMWARE_VERSION    ( ( char* )"v1.6" )
#define FIRMWARE_DATE       ( ( char* )"2019-02-15" )
...
extern PinName SensorNssPin[];

typedef struct
{
    /*uint16_t data_a1;*/
    uint16_t data_a2;       /* @@@ Pressure data    */
    uint16_t data_temp;     /* @@@ Temperature data */
}DemoSensorData_t;

typedef struct
{
    DemoSensorData_t *Sensor;
    uint8_t SensorCount;
    uint8_t LastSensorPos;
}DemoSensors_t;

extern DemoSensors_t *DemoSensors;
extern uint16_t DemoEnabledSensorMask;
extern uint16_t DemoCurrentSensorsMask;

uint8_t InitSensors();
void RefreshSelectedSensors();
void RefreshEnabledSensors();
uint8_t GetSelectedSensorId(uint8_t offset);
uint8_t CountSetBits(uint16_t);
/* @@@ TODO : Merge Rotate Left and Right */
