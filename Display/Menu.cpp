 ...
 * \Page view id
 */
uint8_t PageViewId, PrevPageViewId = 0;
...
GraphObject_t MenuObjects[GRAPH_OBJECTS_COUNT] =
{
    ...
    { TITLE_TEXT_BUTTON,GO_INVISIBLE, 0,   0,   30,     240,   1, BACK_COLOR, BACK_COLOR,    false, NULL, NULL,        false },
    { FIRM_LOGO,        GO_IMAGE,     0,  37,  180,     150,   1, BACK_COLOR, WHITE,         false, NULL, SemtechLogo, false },
...
void MenuInit( void )
{
 ...
 /*@ Handle events occuring in the current page */
uint8_t MenuHandler( bool refresh )
{
    ...
    else if( ( graphObjectTouched != 0xFF ) || ( refresh == true ) )
    {
        switch( CurrentPage )
        {
            ...
            case PAGE_HTSENS: /*@ Handle events in HTSENS Demo page */
                ...
                if( graphObjectTouched == BTN0_BORDER )
                {
                    ...
                    else
                    {
                        ...
                        if( !PageViewId ) DrawText( LINE3_COL0_TEXT, NULL, NULL, ( uint8_t* )"MASTER" );
                    }
                    Eeprom.EepromData.DemoSettings.HoldDemo = true;
                    EepromSaveSettings( DEMO_SETTINGS );
                    MenuSetPage( PAGE_HTSENS );
                    wait_ms( 200 ); /* @@@ debouncing of the screen */
                }
                else if( graphObjectTouched == BTN7_BORDER )
                {
                    StopDemoApplication( );
                ...
                else if( graphObjectTouched == BTN9_BORDER )
                {
                    ...
                    PreviousPage = PAGE_HTSENS;
                    MenuSetPage( PAGE_RADIO_PARAM );
                }
                else if( graphObjectTouched == TITLE_TEXT_BUTTON )
                {
                    PrevPageViewId = PageViewId;        /* Backup last page ID in a different variable, so that MenuSetPage can determine if the current page have changed */
                    PageViewId += 1;                    /* Increment page ID */
                    PageViewId %= 2;                    /* Only 2 pages available for now */
                    MenuSetPage( PAGE_HTSENS );
                }
                else if( graphObjectTouched == BTN1_BORDER ) /* @@@ Button sensor UP */
                {
                    RotateLeftSelectedSensors();
                    PrevPageViewId = 0xff;                         /* Force page refresh */
                    MenuSetPage( PAGE_HTSENS );               
                    // wait_ms( 200 );                          /* @@@ debouncing of the button */
                }
                else if( graphObjectTouched == BTN6_BORDER ) /* @@@ Button sensor DOWN */
                {
                    RotateRightSelectedSensors();
                    PrevPageViewId = 0xff;                         /* Force page refresh */
                    MenuSetPage( PAGE_HTSENS );               
                    // wait_ms( 200 );                          /* @@@ debouncing of the button */
                }
                else if( refresh == true )                   /* Refresh required by Application */
                {
                    MenuSetPage( PAGE_HTSENS );
                }
                break;
...
{
    if( (page != CurrentPage) || (PageViewId != PrevPageViewId) )
    {
    ...
    switch( page )
    {
        case START_PAGE:
            ...
            DrawActiveButton( BTN6_BORDER, ( uint8_t* )"DEMO HTSENS Rake", NULL );
            DrawActiveButton( BTN7_BORDER, ( uint8_t* )"Utilities", NULL );
            DrawActiveButton( BTN8_BORDER, ( uint8_t* )"Radio Set", NULL );
            CurrentPage = START_PAGE;
            break;

        /* @@@ Handle page refresh triggered by the HTSENS demo application */
        case PAGE_HTSENS:
            if( (CurrentPage == PAGE_HTSENS) && (PageViewId == PrevPageViewId) ) /* @@@ Same page as previous, only refresh the graphicals components that may change */
            {
                if( Eeprom.EepromData.DemoSettings.HoldDemo == true )
                {
                    ButtonChangeText( BTN8_BORDER, ( uint8_t* )"   START", NULL );
                ...
                if(PageViewId)  /* @@@ "Sensor" View */
                {
                    char str1[40], str2[40], str3[40], str4[40];
                    
                    uint16_t tempSensorValue;
                    
                    tempSensorValue = DemoSensors->Sensor[GetSelectedSensorId(0)].data_a2;
                    if( ( tempSensorValue != 0x0000 ) && ( tempSensorValue != 0xffff ) )
                    {
                        sprintf(str1, "%.3f", (((double)tempSensorValue*0.0008)-(0.90))); // Pressure sensor 1
                    }
                    else
                    {
                        strcpy(str1, "N/A");
                    ...
                    tempSensorValue = DemoSensors->Sensor[GetSelectedSensorId(0)].data_temp;
                    if( ( tempSensorValue != 0x0000 ) && ( tempSensorValue != 0xffff ) )
                    {
                        sprintf(str2, "%.3f", (((double)tempSensorValue*0.0184)-(242.88)));   // Temp sensor 1
                    }
                    else
                    {
                        strcpy(str2, "N/A");
                    ...
                    tempSensorValue = DemoSensors->Sensor[GetSelectedSensorId(1)].data_a2;
                    if( ( tempSensorValue != 0x0000 ) && ( tempSensorValue != 0xffff ) )
                    {
                        sprintf(str3, "%.3f", (((double)tempSensorValue*0.0008)-(0.90))); // Pressure sensor 2
                    }
                    else
                    {
                        strcpy(str3, "N/A");
                    ...
                    tempSensorValue = DemoSensors->Sensor[GetSelectedSensorId(1)].data_temp;
                    if( ( tempSensorValue != 0x0000 ) && ( tempSensorValue != 0xffff ) )
                    {
                        sprintf(str4, "%.3f", (((double)tempSensorValue*0.0184)-(242.88)));   // Temp sensor 2
                    }
                    else
                    {
                        strcpy(str4, "N/A");
                    ...
                    DrawText( LINE3_COL0_TEXT, NULL, ( uint8_t* )str1, ( uint8_t* )" Bar" );
                    DrawText( LINE4_COL0_TEXT, NULL, ( uint8_t* )str2, ( uint8_t* )" Deg C" );
                    DrawText( LINE6_COL0_TEXT, NULL, ( uint8_t* )str3, ( uint8_t* )" Bar" );
                    DrawText( LINE7_COL0_TEXT, NULL, ( uint8_t* )str4, ( uint8_t* )" Deg C" );
                }
                else    /* @@@ "Stats" View */
                {
                    if( Eeprom.EepromData.DemoSettings.Entity == MASTER )
            ...
            else /* @@@ Not the same page as previous. We need to set initial values for all visual components */
            {
                InitSensors();
                ...
                {
                    DrawActiveButton( BTN8_BORDER, ( uint8_t* )"   STOP", NULL );
                }
                
                DrawActiveButton( TITLE_TEXT_BUTTON, NULL, NULL );
                DrawActiveButton( BTN7_BORDER, ( uint8_t* )"    EXIT", NULL );
                DrawActiveButton( BTN9_BORDER, ( uint8_t* )"  SETTINGS", NULL );
                
                if( PageViewId ) /*@ "Sensors" View */
                {
                    DrawText( TITLE_TEXT, ( uint8_t* )"HTSENS LoRa - Sensor View", NULL, NULL );
                    
                    DrawActiveButton( BTN1_BORDER, ( uint8_t* )"            UP", NULL );
                    DrawActiveButton( BTN6_BORDER, ( uint8_t* )"           DOWN", NULL );

                    const char *SensorStr = "Sensor ";
                    const char *SensorStr2 = " of "; 
                    char SensorCount[16];
                    char tempoStr[12];
                    char SensorIdStr[4];
                    int8_t i = 0;
                    
                    sprintf(SensorCount, "%d", GetSelectedSensorId(DemoSensors->SensorCount) + 1); 
                    
                    if( GetSelectedSensorId(0) != 0xFF ) i++;
                    if( GetSelectedSensorId(1) != 0xFF ) i++;
                
                    /* @@@ Print second line only if 2 sensors are selected */
                    if(i > 0) 
                    {
                        strcpy(tempoStr, SensorStr);
                        sprintf(SensorIdStr, "%d", GetSelectedSensorId((uint8_t) --i)); 
                        strcat(tempoStr, SensorIdStr); 
                        //strcat(tempoStr, SensorStr2);
                        //strcat(tempoStr, SensorCount);
                        DrawText( LINE5_COL0_TEXT, ( uint8_t* )tempoStr, NULL , NULL );
                        DrawText( LINE6_COL0_TEXT, ( uint8_t* )"Pressure   :", ( uint8_t* )"  N/A", NULL );
                        DrawText( LINE7_COL0_TEXT, ( uint8_t* )"Temp       :", ( uint8_t* )"  N/A", NULL );
                    }
                    ...
                    if(i > 0) 
                    {
                        ...
                        //strcat(tempoStr, SensorStr2);
                        //strcat(tempoStr, SensorCount);
                        DrawText( LINE2_COL0_TEXT, ( uint8_t* )tempoStr, NULL , NULL );
                        DrawText( LINE3_COL0_TEXT, ( uint8_t* )"Pressure   :", ( uint8_t* )"  N/A", NULL );
                        DrawText( LINE4_COL0_TEXT, ( uint8_t* )"Temp       :", ( uint8_t* )"  N/A", NULL );  
                    }
                }
                else /* @@@ "stats" View */
                {
                    DrawText( TITLE_TEXT, ( uint8_t* )"HTSENS LoRa - Stats View", NULL, NULL );
                    if( Eeprom.EepromData.DemoSettings.Entity == MASTER )
                    {
                        DrawText( LINE3_COL0_TEXT, NULL, NULL, ( uint8_t* )"MASTER" );
                    ...
                    DisplayCurrentRadioParams( PAGE_HTSENS );
            ...
            CurrentPage = PAGE_HTSENS;
            PrevPageViewId = PageViewId;
            break;
 ...
 */
void DrawActiveButton( uint8_t buttonId, uint8_t* text1, uint8_t* text2 ) /* @@@ DrawActiveButton overwrite TouchActive */
{
